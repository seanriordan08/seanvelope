class BasinMetrics::PartsController < ApplicationController
  ...
  def part_params
    if params[:basin_metrics_part]
      params.require(:basin_metrics_part).permit(:name, :type, :number, :size, :quantity, :revenue, :well_id)
    else
      params.permit(:id, :name, :type, :number, :size, :quantity, :revenue, :well_id)
    end
  end

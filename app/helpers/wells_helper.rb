module WellsHelper
  ...
  def get_background_colors
    # customer_count = current_user.company.wells.order(:name).collect(&:name).size
    background_colors = []
    24.times {background_colors << 'rgba(75, 192, 192, 0.2)'}
    background_colors
  ...
  def get_border_colors
    # customer_count = current_user.company.wells.order(:name).collect(&:name).size
    border_colors = []
    24.times {border_colors << 'rgba(75, 192, 192, 1)'}
    border_colors
